...
def editGun(guncompany_id):
    ...
    if editedGun.user_id != login_session['user_id']:
		return "<html><script>alert('you are not allowed to edit\
        this restaurant.Please create your own\
        restaurant to edit.');</script></html>"
    if request.method == 'POST':
...
def showMenu(guncompany_id):
    ...
    items = session.query(GunModel)\
        .filter_by(guncompany_id=guncompany_id).all()
    if 'username' not in login_session or \
            creator.id != login_session['user_id']:
        session.close()
        return render_template('publicarms.html', items=items,
                               guncompany=guncompany, creator=creator)
    else:
        return render_template('main.html', items=items, guncompany=guncompany,
                               creator=creator)
...
def deleteGunModel(guncompany_id, menu_id):
    ...
    if login_session['user_id'] != guncompany.user_id:
		return "<script>function myFunction() {alert('You are not authorized\
        to delete this gun model. Please create your own gun model\
